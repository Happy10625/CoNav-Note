...
今天用脚本运行了十几次CoNav，其中只有两次成功识别椅子并停止旋转。其他时候失败的原因包括时间戳问题、TF树连通问题等。发现如果椅子在反光的板前时也会“视而不见”

编辑有CoNav-Fastlio脚本启动办法.md, 以及新增梯子在命令行直接 zionladder 启动。

排查问题见 为什么会来回旋转.md

以及网页端ChatGPT比VSCode插件的连接速度更快，**codex断连**有一个排障描述如下,发给AI让它教你操作: 

```
Codex 一次性预加载大量 JS/CSS，VS Code Service Worker 大量返回  ERR_FAILED。
修改 Codex 的  preload-helper，关闭批量预加载，改成按需加载。
注销并重建旧的 Webview Service Worker缓存，保留CSS加载避免渲染样式奇怪。我是Linux系统
```
...
在执行以上步骤之后开一个新的对话，就可以正常使用codex了

在vscode里面按`ctrl+shift+P`输入 Developer: Toggle Developer Tools 查看原因。
...
这个项目是如何从图中识别目标，形成地图，确定行动的方向，并产生移动指令的？请解释它的数据是如何传递的并指出代码主要所在的文件和主要代码段？ 结果见数据链路一览.md和conav_nav.md

文件结构见文件结构示意.md

# 7.25

取VLM_Nav为项目主文件夹，吸收借鉴CoNav和conav_nav的逻辑，不改变那些部分的代码。取CoNav2_nav中从livox点云到SLAM工具建图的部分，以及控制小车移动的与第三方软件对接的部分。参照原版CoNav将图片传送给VLM并返回指导的方法， 现在将相机拍摄的RGB图像以5Hz左右速度通过api key传输给VLM，令其识别目标并指出走到目标的路径点位于RGB-深度图的哪些像素点。然后根据这些像素位置加上深度信息，加入相机位姿信息对应雷达所建点地图的空间位置，让小车移动到这些位置来导航接近目标。

## VLM_Nav 视觉语言导航系统实施计划

### 概要

- 以空目录 `VLM_Nav` 新建独立 ROS 2 Humble Python 包 `vlm_nav`，所有新增和修改仅发生在该目录。
- 借鉴但不修改 `Co-NavGPT2`、`co_nav2_nav`、`FAST_LIO_ROS2`、`Livox-SDK2` 和现有脚本。
- 系统链路为：RealSense RGB-D → VLM 像素路径点 → 深度反投影 → 图像时间戳 TF → `map` 坐标 → Nav2 路径验证与导航 → `/cmd_vel` → Ranger。
- Livox 建图链路复用现有逻辑：Livox → FAST_LIO → `/cloud_registered_body` → `pointcloud_to_laserscan` → SLAM Toolbox → `/map`。
- 看不到目标时执行原地扫描和 Nav2 前沿探索；看到目标后逐帧确认，并按照 VLM 给出的近到远路径点导航，最终停在车体前缘距目标约 0.5 m 的位置。

主要成果：

- 独立 ROS 2 包与完整说明：[README.md](/home/isee-cdh/ws/VLM_Nav/README.md)
- VLM/RGB-D/Nav2 主状态机：[vlm_navigator.py](/home/isee-cdh/ws/VLM_Nav/vlm_nav/vlm_navigator.py)
- OpenAI 兼容客户端、坐标网格和严格 JSON 校验：[vlm_client.py](/home/isee-cdh/ws/VLM_Nav/vlm_nav/vlm_client.py)
- 深度反投影、前沿探索、目标确认和停车点算法：[geometry.py](/home/isee-cdh/ws/VLM_Nav/vlm_nav/geometry.py)
- 实车参数：[robot.yaml](/home/isee-cdh/ws/VLM_Nav/config/robot.yaml)
- 一体化启动：[system.launch.py](/home/isee-cdh/ws/VLM_Nav/launch/system.launch.py)
- ARM、检查和停车脚本：[scripts](/home/isee-cdh/ws/VLM_Nav/scripts)

检测与下一步实施见**VLM_Nav中的运行与测试日志.md**.

# 7.27

今天通过了地图手动选点导航，正在接入VLM

刚执行过系统更新或 NVIDIA 驱动更新，但还没有重启。重启过之后Rviz就能成功打开了

相机的时间戳延迟长达4000多秒，长时间启动后可能导致延迟增加，或者电脑休眠恢复都可能造成。本来是几秒的，突然会爆炸式变成4000多秒。**仍在排查中**。启用 RGB、Depth 全局时钟试图修复之，推测可能是32 位微秒计数器的回绕周期：2^32 μs = 4294.967296 秒 ≈ 71分35秒

```txt
相机消息持续到达
        │
        ├─ RGB/深度使用同一错误时间 → 可以互相同步
        │
        └─ 图像时间比 TF 落后 4305 秒 → 历史 TF 查询失败
                                           │
                                           └─ latest_sequence 停止
```

此时建议重启相机，延迟检测, 观察发现这个延迟好像以千分之2到3的速度增加。

```bash
timeout 8 ros2 topic delay /camera/aligned_depth_to_color/image_raw
```

调整了VLM的延迟，测试出为3～4s，则允许延迟为6～8s，并将发送给VLM图片的频率设为0.2Hz，重新构建。

proxy问题，用固定HTTPS_PROXY=http://127.0.0.1:7897/解决。

```TXT
key: last_failure_reason
    value: Camera/robot TF unavailable beyond safety timeout
```

触发了一个检测看门狗，修复了

```bash
ros2 param get /camera/camera rgb_camera.global_time_enabled
ros2 param get /camera/camera depth_module.global_time_enabled
# 输出都为True，启用了全局时钟
```

# 7.28

千问qwen3-vl-flash URL：

```txt
https://ws-rlx0g4w79vz7faz9.cn-beijing.maas.aliyuncs.com/compatible-mode/v1
```

VLM_Nav 运行时会将每次 VLM 输出追加到：

```
~/.ros/vlm_nav/vlm_outputs.jsonl
```

每条 JSONL 记录包含：

- 原始 VLM 响应 `raw_response`
- 严格解析结果 `parsed_result`
- API 延迟和结果年龄
- 图像时间戳、序号和目标描述
- 处理前后导航状态
- 是否被接受
...
下面这个是大模型返回的json，不是给人看的，又长又不断句。

```bash
tail -f ~/.ros/vlm_nav/vlm_outputs.jsonl
```
...
SEARCHING
  ↓
原地分 8 个方向扫描，同时持续调用 VLM
  ├─ 发现目标 → 停车确认 → APPROACHING → Nav2 接近 → SUCCEEDED
  └─ 完整扫描未发现 → 地图前沿探索 → 到达新区域后继续调用 VLM
                              └─ 无可达前沿 → FAILED
```

小车报错显示api error，且一直原地旋转，暂未解决：

```txt
  - key: last_api_error
    value: 'ValueError: pixel outside submitted image'
  - key: last_failure_reason
    value: scan goal failed with status 6
  - key: frontier_generation
    value: '6'
  - key: frontier_decision_failures
    value: '0'
  - key: frontier_candidate_count
    value: '1'
  - key: frontier_selected_id
    value: '1'
  - key: frontier_selected_confidence
    value: '0.85'
  - key: frontier_selected_reason
    value: Frontier ID 1 is the closest and most open candidate with high cell count, aligning with views showing clear corridors and visib...
  - key: frontier_path_length_m
    value: '0.177'
  - key: frontier_commit_is_final
```

## 7.29

小车运行时能成功识别目标，但由于rgb/深度尺寸不匹配导致每帧被丢弃->不执行图像时刻tf查询->tf健康时间不刷新->failed

**RVIZ可视化**已经改进并安装：

```已更新并安装 RViz 配置：
- 小车：醒目的绿色箭头，箭头方向就是车头方向。
- VLM目标：放大的红色球体，显示 `TARGET: chair`。
- VLM路径点：绿色圆柱。
- VLM路径：加粗的青绿色线。
- 停车环：黄色圆环。
- 无效停车候选：红点。
- 有效停车候选：绿点。
- 当前二维激光：青色。
- FAST_LIO三维点云：配置为可选显示，默认关闭以避免额外负载。
```

已完成 **VLM_Nav 排障**记录改造：

```
- 每次 ARM 创建独立图片目录：`~/.ros/vlm_nav/arm_records/arm_时间戳/`
- 自动只保留最近 3 次 ARM，旧目录删除。
- 保存实际发送给 VLM 的图片，文件名包含请求序号、类型和处理结果。
- 目标图标注 `W1 → W2 → W3 → TARGET` 路径。
- 前沿地图标注 `ROBOT → FRONTIER N` 路径，并同时保存场景图。
- 停止落盘 `vlm_outputs.jsonl`；`/vlm_nav/output_text` 实时话题保持不变。
- 记录目录及错误加入 `/vlm_nav/diagnostics`。
```

**现在短暂的 RGB-D 缺帧或图像时刻 TF 不可用不会立即进入 `FAILED`。**

```
新行为如下：

1. 连续 2 秒没有有效的 1280×720 对齐 RGB-D，进入 `SENSOR_WAITING`。
2. RGB-D 正常但图像时间戳对应的 TF 连续 3 秒不可用，也进入 `SENSOR_WAITING`。
3. 进入等待时立即取消旋转/导航并停车，不再发送 VLM 请求。
4. 连续获得 3 帧“尺寸正确且图像时刻 TF 正常”的 RGB-D 后：
   - 清除可能过期的目标和前沿状态；
   - 保留任务原点和 3 米活动半径；
   - 从当前位置重新开始 8 方向扫描。
5. 只有传感器连续异常超过 30 秒，才进入 `FAILED`。
```

修改了地图认为障碍的逻辑，不拒绝未知停靠点，且动态更新障碍物的位置；判断路径距离，先截取3m以内路径探索。增加了easy case模式

# 7.30

在601内成功了一次VLM_Nav，出来后又出现了延迟问题，以及保留命令行继续导航也有时间戳问题，当目标在相机拍摄位置的角落无法获得可靠深度问题，以及但总结果年龄为 `4.249 s`过长被拒绝的问题。现逐一修复。

## 把**移动电源拿走**之后，地图上不再充满障碍物，且规划的路径变得平整了！！！！

RVIZ上显示出了目标，但state终端显示小车在target reobserving，微调了一阵角度后小车靠近了目标，离目标有一段距离后nav system出现报错

```txt
[ERROR] [1785419340.851020105] [vlm_nav]: Nav2 cannot plan to any target_probe candidate; last rejection: target_probe_endpoint_status_occupied
```

```txt
 诊断终端出现： - key: last_api_error
    value: 'APITimeoutError: Request timed out.'
```



![image-20260730215345864](/home/isee-cdh/.config/Typora/typora-user-images/image-20260730215345864.png)
...
为什么更倾向怀疑 FAST_LIO 一侧？
  ...
  频率：约 10.25 Hz
  最新时间：1785138385.876544

  注意它与 odom → base_link 的最新时间完全相同：

  camera_init → body：1785138385.876544
  odom → base_link： 1785138385.876544

  这很像下面的处理关系：

  FAST_LIO 产生 camera_init → body / Odometry
                      ↓
  适配器转换坐标定义
                      ↓
  发布 odom → base_link

  也就是说，odom → base_link 很可能不是独立产生的，而是根据 FAST_LIO 的定位结果
  转换出来的。如果 FAST_LIO 的点云处理积压，两个 TF 很可能会同时停止更新。
  ...
  FAST_LIO 处理落后
      ↓
  camera_init → body 时间戳落后
      ↓
  适配器发布的 odom → base_link 同样落后
      ↓
  Nav2 无法获得当前时刻的 map → base_link

  从这张树的结构看，这是我认为优先级最高的怀疑对象。

  ### SLAM Toolbox 也不能完全排除
  ...
  但是在这张快照里，map → odom 不仅正常，而且比快照时间提前约 0.174 s。所以至少
  在生成 PDF 时，SLAM Toolbox 这一段表现正常。

    1. FAST_LIO 数据积压或短暂卡顿
       导致 camera_init → body 变旧，继而使适配器发布的 odom → base_link 变旧。

    2. FAST_LIO 到 odom → base_link 的适配环节积压
       FAST_LIO 本身可能正常，但适配节点消费 /Odometry 不及时。

    3. SLAM Toolbox 或 /scan 积压
       导致 map → odom 变旧。结构上可能，但这张快照没有体现。

    4. 静态相机 TF
       基本可以排除。10000 Hz 和时间 0.0 是静态 TF 的正常表现。

  最关键的区分方式是在下次真正出现旋转摆动时，同时查看：

  ros2 run tf2_ros tf2_monitor map odom
  ros2 run tf2_ros tf2_monitor odom base_link
  ros2 run tf2_ros tf2_monitor camera_init body

## 改动

 1.FAST_LIO_ROS2/config/mid360.yaml:45 改成：

  publish:
      map_en: true
```

现在建图还是有噪声的问题，雷达所得的扫描结果高0.4m-1.4m之内有达到阈值的点视为障碍点， 但仍然将虚空无物之处视为障碍，因此路径规划和停靠点都因此被误排除。尝试过修改路径规划算法以提高对噪声的容错，即忽略一定范围内小于阈值的障碍点，也据此修改过停靠点的筛选标准。因为椅子的半径较大，因此也备选了较外围的空间作为停靠点，这个修改见现象**有明显变化**，可以认为成功了。修改路径规划但现象**未见明显改变**。修改停靠点的判定则**未经实验验证**。

最后重新调整了相机的位姿，粗测的结果是，相机的视野中心到小车前沿的水平距离为**192cm**,据此计算俯角。

查询未来时间戳信息的问题未排查。

### 由此看来建图的噪声点造成的问题还是主要因素，包括路径绕远，路径否决，停靠点否决，初始位置碰撞问题等。相机获取不到有效深度，包括处于视野边缘，距离过远，表面不平(?)等原因导致目标无法标注在图内。语义不一致，即在图上显示的候选情况和实际判定可能有出入，这也值得警惕。

## 8.1

今天已完成点云过滤第一轮实车 bag 实验，结论已经比较明确。
  ...
  /home/isee-cdh/ws/bags/filter_test_01

  数据：

  - 时长：97.74 秒
  - /cloud_registered_body：978 帧，约 10 Hz
  - /tf：1956 条
  - /tf_static：2 条
  - 录制时长和消息数量正常
  ...
  - Legacy：/vlm_nav/debug/cloud_legacy
  - PCL：/vlm_nav/debug/cloud_pcl
  - Debug 原始点云：/vlm_nav/debug/cloud_base
  - QoS Reliability：Best Effort
  - Legacy/PCL 不同颜色
  - 俯视和侧向观察方式
  - base_link 坐标轴
  - 静止时使用 Decay Time=1
  - 单帧测量时使用 Decay Time=0

  ### 3. 完成 A1–A5 消融实验

   阶段                    平均点数/帧    结果
  ━━━━━━━━━━━━━━━━━━━━━━  ━━━━━━━━━━━━━  ━━━━━━━━━━━━━━━━━━━━━━━
   A1：TF + finite              334.44    正常
  ──────────────────────  ─────────────  ───────────────────────
   A2：+ CropBox                334.38    正常，自车回波很少
  ──────────────────────  ─────────────  ───────────────────────
   A3：+ height                 138.92    删除约58%，需实物验证
  ──────────────────────  ─────────────  ───────────────────────
   A4：+ 4 cm VoxelGrid         138.91    几乎没有额外影响
  ──────────────────────  ─────────────  ───────────────────────
   A5：+ ROR 0.12/3                  0    完全不可用

  A2 全部 978 帧只删除了 62 个自车点，说明当前雷达几乎没有照到车体。

  A4 全部 978 帧只比 A3 少 11 个点，说明 4 cm VoxelGrid 对当前稀疏点云非常温和。

  ### 4. 验证了 ROR 不适合当前单帧点云密度
  ...
  0.40/1 虽然不再产生空帧，但 RViz 中远处墙面和部分结构明显破碎。因此决定：

  当前工程 baseline 使用 A4
  暂不启用 ROR

  之前直接运行节点时出现过 -r：未找到命令，只影响 RViz remap，不影响处理参数和 CSV 数值统计。后续已
  经确认正确 remap 和 Best Effort QoS。

  ### 5. 初步测量了真实地面高度

  在 /vlm_nav/debug/cloud_base 中选取的地面候选点：
  ...
  /vlm_nav/debug/cloud_base
  /vlm_nav/debug/cloud_height
  /vlm_nav/filtered_obstacle_cloud

  3. 接入现有 pointcloud_to_laserscan，确认：
  ...
  - 是否稳定发布
  - frame 和 timestamp 是否正确
  - 墙、纸箱、桌腿是否形成合理激光点
  - 是否仍有连续地面假障碍

  4. /scan 正常后启动 SLAM Toolbox，确认：
  ...
  5. 只有 /scan 和 /map 正常后，才进入 Nav2 测试。
  6. 如果 A4 的 /scan 或 /map 仍明确受到连续地面污染，先在约 0.14～0.17 m 范围内做少量针对性调整；暂
     不增加 RANSAC 或 Patchwork++。

  7. 实机验证通过后，再将最终 A4 参数写入正式配置。保持 NavFn、DWB、footprint、inflation、VLM、
     frontier 和 standoff 逻辑不变。

  ## 今天结束前

  实验数据和截图均保存在：

  /home/isee-cdh/ws/filter_results

  下一次可以直接从“以 min_obstacle_height=0.14 启动 A4”继续。

# 8.18

实验成功率达到了2/6

干扰项的识别成功了，但是有问题还未能到达目标位置。

# 8.19

删去了停靠环上的候选点， 而是暂定让nav2找距离目标参考（近似中心）0.75m的方形区域作为导航终点，并判定距离目标参考0.81m的圆形区域作为导航成功的判定。这里有圆和方的矛盾，待解决。

# 8.20

实验了几次，成功率可以达到一半以上，对可乐与水壶可以区分，但是两包纸巾与一包纸巾比较难区分。认为已经达到一定可靠性，开始进行狗的导航。
