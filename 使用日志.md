...
```
Codex 一次性预加载大量 JS/CSS，VS Code Service Worker 大量返回  ERR_FAILED。<br/>修改 Codex 的  preload-helper   ，关闭批量预加载，改成按需加载。<br/>注销并重建旧的 Webview Service Worker  缓存，保留CSS加载避免渲染样式奇怪。我是Linux系统
```
